from batch_entmax_gaussian_kernels import TriweightGaussian1DKernel
import torch
...
class ContinuousTriweightFunction(torch.autograd.Function):
    @classmethod
    ...
    def forward(cls, ctx, theta, psi):
        ...
        mu = theta[:, 0] * sigma ** 2
        kernel = TriweightGaussian1DKernel(mu, sigma**2)
...
class ContinuousTriweight(nn.Module):
    ...
    def forward(self, theta):
        return ContinuousTriweightFunction.apply(theta, self.psi)
